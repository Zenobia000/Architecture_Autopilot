...
references:
  ...
  - devteam_knowledge_base/04_freeze_gates.md
...
寫入 `.claude/context/devteam/indexes/feature_index.json`、`topic_index.json`、`catalog_usage.json`，內容均為 `{}`。
這些 index 是 derive 自 adr-ledger + ADR / DR frontmatter 的 `Tags:` / `Feature:` / `Related KB:`，後續每次 driver 寫入新 ADR / DR 後 router 應 rebuild。完整 spec / schema 範例見 `devteam_knowledge_base/03_document_templates.md` 「ADR / DR 索引機制」段。
...
3. 重寫：
   - `feature_index.json`：以 feature-slug 為 key，值為該 feature 相關的 ADR/DR 條目陣列
   - `topic_index.json`：以 tag 為 key，值為相關 ADR/DR 條目陣列
   - `catalog_usage.json`：以 KB 檔名為 key，值為引用該 KB 的 ADR/DR 條目陣列（含 section）
4. 若 ADR / DR 缺 Tags / Feature / Related KB → 不阻擋寫入，但於 Phase 5a critique 時請 arch persona 補

**用途**：
- 新 driver 接手某 feature 時，先讀 `feature_index.json[feature]` 取已決議 ADR 清單，避免重複決議
- 新 ADR 起草前，arch driver 讀 `topic_index.json[topic]` 取相關 ADR，避免衝突
- `catalog_usage.json` 累積成 case law，亦可發現未被引用的 KB 段（設計失敗信號）
